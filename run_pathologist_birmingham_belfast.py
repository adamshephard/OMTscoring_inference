...
cohort = 'oed'
method = 'pathologist_corrected_belfast'
features = "binary" #'who_mild' #'binary'
...
external = True
subcohort = "belfast" #None #'belfast'

sub_cohorts = {
    'birmingham': 5,
    'belfast': 6,
}
# who compact wrong for birmingham - corrected now
...
if external == False:
    ...
    if features == 'who_mildmod':
        ftr = 'who_compact'
        # ftr = 'who'
    elif "who" in features:
        ftr = 'who'
    
    ftr = features

if subcohort is not None:
    temp_output = os.path.join(temp_output, subcohort)
...
for repeat in range(1, repeats+1):   # number of folds
    for fold in range(folds):      # number of runs for each fold
        ...
        if subcohort is not None:
            test_data = test_data.loc[test_data['cohort']==sub_cohorts[subcohort]]
