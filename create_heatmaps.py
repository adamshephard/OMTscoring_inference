...
from dataloader.mil_reader import featuresdataset_wsi_mlp_inference
...
from utils.patch_generation import create_morph_patches
...
input_root = f'/data/ANTICIPATE/outcome_prediction/MIL/idars/output/cohort_1,3,4/transformation/{method}/{features}_100eps/oed/'
    ...
    for fold in range(nr_folds):      # number of runs for each fold
        ...
        for r, r_data in test_data.iterrows():
            ...
            if cohort == 1:
                ...
                mask_path = input_segs[2]
            elif cohort == 3:
                wsi_path = glob.glob(os.path.join(input_wsis[0], case + '*'))[0]
                mask_path = input_segs[0]
            elif cohort == 4:
                wsi_path = glob.glob(os.path.join(input_wsis[1], case + '*'))[0]
                mask_path = input_segs[1]
            
            if os.path.exists(os.path.join(out_folder, case+'.png')):
                continue
            ...
            #loading data
            # train set
            test_dset = featuresdataset_wsi_mlp_inference(
                patches, rois,
                layer_col_dict=COL_DICT,
                )
